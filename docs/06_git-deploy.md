...
이번 chapter에서는 파일을 GitHub에 올리고 인터넷에 게시하는 방법을 익힌다.
...
예를 들어 GitHub의 username(로그인 메일 계정을 말하는 것이 안니다.)이 `hong-gildong`이면 `hong-gildong.github.io`으로 web site의 주소가 설정된다.
...
5. **Configuration -> Add .gitignore -> no .gitignore** (Git이 무실할 파일 목록)
6. **Configuration -> Add license -> No license** (공개 파일의 사용 조건명시 파일)
...
### 파일 올리기
local에서 작성했던 Mkdocs 설정 파일을 Github에서 직접 upload한다. (Quick setup을 이용)  
작성한 파일이 없다면 creating a new file을 이용한다.

### 폴더 구조 확인

올리기 전에 MkDocs 프로젝트 폴더 구조를 확인한다.
...
repository에 파일을 upload한 후에 Markdown 파일을 HTML로 변환하는 작업이 필요하다.
...
jobs:
  deploy:
    runs-on: ubuntu-latest      # 작업이 샐행되는 OS
    steps:
...
Actions가 성공하면 1~2분 후 브라우저에서 접속한다.

```
https://<username>.github.io
```

이후에는 파일을 수정해서 올리기만 하면 Actions가 자동으로 사이트를 업데이트한다.
